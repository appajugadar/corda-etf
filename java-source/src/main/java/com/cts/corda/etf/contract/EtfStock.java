...
public class EtfStock implements Contract {

    public static final String ETF_STOCK_CONTRACT = "EtfStock";
    ...
    public TransactionBuilder generateIssue(@NotNull PartyAndReference issuance, String etfName, Long quantity, @NotNull Party notary, Integer encumbrance) {
        State state = new State(issuance, issuance.getParty(), etfName, quantity);

        TransactionState output = new TransactionState<>(state, ETF_STOCK_CONTRACT, notary, encumbrance);
    ...
    public void generateMove(TransactionBuilder tx, StateAndRef<State> security, AbstractParty newOwner) {
        ...
        tx.addOutputState(new TransactionState<>(new State(security.getState().getData().getIssuance(), newOwner, security.getState().getData().getEtfName(), security.getState().getData().getQuantity()),
                ETF_STOCK_CONTRACT, security.getState().getNotary(), security.getState().getEncumbrance()));
    ...
    public static class State implements OwnableState {
        ...
        public State(PartyAndReference issuance, AbstractParty owner, String etfName, Long quantity) {
        ...
        public State copy() {
            return new State(this.issuance, this.owner, this.etfName, this.quantity);
        }

        @NotNull
        @Override
        public CommandAndState withNewOwner(@NotNull AbstractParty newOwner) {
            return new CommandAndState(new Commands.Move(), new State(this.issuance, newOwner, this.etfName, this.quantity));
        }
        ...
        public boolean equals(Object that) {
            ...
            if (etfName != null ? !etfName.equals(state.getEtfName()) : state.getEtfName() != null)
                return false;
        ...
        State withoutOwner() {
            return new State(issuance, new AnonymousParty(NullKeys.NullPublicKey.INSTANCE), etfName, quantity);
        }
