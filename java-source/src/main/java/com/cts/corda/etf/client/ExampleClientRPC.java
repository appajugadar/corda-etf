...
public class ExampleClientRPC {
    ...
    public static void main(String[] args) throws ActiveMQException, InterruptedException, ExecutionException {
        ...
        final DataFeed<Vault.Page<SecurityBuyState>, Vault.Update<SecurityBuyState>> dataFeed = proxy.vaultTrack(SecurityBuyState.class);
        final Vault.Page<SecurityBuyState> snapshot = dataFeed.getSnapshot();
        final Observable<Vault.Update<SecurityBuyState>> updates = dataFeed.getUpdates();
